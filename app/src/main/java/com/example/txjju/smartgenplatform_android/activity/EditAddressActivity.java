...
public class EditAddressActivity extends AppCompatActivity implements View.OnClickListener {
    private TextView tvName,tvPhone,tvAddress,tvDelete;
    private String name,phone,address;
    private Button btnSave;
    public final static String TAG = "EditAddressActivity";
    private User user;
    private int userId;//保存用户ID
    public String purchaseCartId;//保存产品ID
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        purchaseCartId = intent.getStringExtra("purchaseAddressId");
        Log.i(TAG,"地址"+purchaseCartId);
        init();
        LoadData();
    }

    private void init() {
        tvName = findViewById(R.id.tv_edit_address_name);
        tvPhone = findViewById(R.id.tv_edit_address_phone);
        tvAddress = findViewById(R.id.tv_edit_address_address);
        tvDelete = findViewById(R.id.tv_delete);
        btnSave = findViewById(R.id.btn_edit_address_save);
    ...
    private void LoadData() {
        ...
        formBody.add("queryParam.condition","id="+purchaseCartId);//获取用户收货详细信息
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Log.d("EditAddressActivity","确认订单：获取数据失败了");
            }
            @Override
                        ...
                        public void run() {//调回到主线程
                            ...
                            try {
                                ...
                                basePojo = JsonUtil.getBaseFromJson(
                                        EditAddressActivity.this, addressResult, new TypeToken<BasePojo<Purchaseaddress>>(){}.getType());
                                if(basePojo != null){
                                    if(basePojo.getSuccess() && basePojo.getTotal() > 0){   // 信息获取成功,有数据
                                        ...
                                        tvName.setText(list.get(0).getPuraddressUserName());
                                        tvPhone.setText(list.get(0).getPuraddressUserPhone());
                                        tvAddress.setText(list.get(0).getPuraddressAddress());
                                    }else{
                                        ToastUtils.Toast(EditAddressActivity.this,basePojo.getMsg(),0);
                                    }
                                }
                            } catch (IOException e) {
                                e.printStackTrace();
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.tv_delete:
                deleteAddress();
                break;
            case R.id.btn_edit_address_save:
    ...
    private void updateAddress() {
        ...
        OkHttpClient client = new OkHttpClient();//创建OkHttpClient对象。
        FormBody.Builder formBody = new FormBody.Builder();//创建表单请求体
        Log.i(TAG,"修改收货地址ID"+purchaseCartId);
        formBody.add("id",purchaseCartId);//获取用户收货详细信息
        ...
        Request request = new Request.Builder()//创建Request 对象。
                .url(Constant.USER_UPDATEADDRESS)
                .post(formBody.build())//传递请求体
                .build();
        client.newCall(request).enqueue(new Callback() {
            @Override
    ...
    private void deleteAddress() {
        ...
        OkHttpClient client = new OkHttpClient();//创建OkHttpClient对象。
        FormBody.Builder formBody = new FormBody.Builder();//创建表单请求体
        Log.i(TAG,"删除收货地址ID"+purchaseCartId);
        formBody.add("id",purchaseCartId);//获取用户收货详细信息
        Request request = new Request.Builder()//创建Request 对象。
                .url(Constant.USER_DELETEADDRESS)
                .post(formBody.build())//传递请求体
                .build();
        client.newCall(request).enqueue(new Callback() {
            @Override
